...
import { assignmentService, quizService, announcementService, scheduleService, subjectService, classService } from '../../services';
...
export const TeacherCalendar = () => {
  ...
  useEffect(() => {
    const loadEvents = async () => {
      try {
        ...
        const [subjectsData, classesData, announcementsData] = await Promise.all([
          subjectService.getSubjects(undefined, user?.id),
          classService.getClasses(),
          announcementService.getAnnouncements({ targetAudience: 'teacher' }),
        ]);

        // Get class IDs from subjects
        ...
        // Load assignments, quizzes, and schedules for teacher's classes
        const [assignmentsData, quizzesData, schedulesData] = await Promise.all([
          classIdsArray.length > 0 ? Promise.all(classIdsArray.map(classId => assignmentService.getAssignments(classId))).then(results => results.flat()) : Promise.resolve([]),
          classIdsArray.length > 0 ? Promise.all(classIdsArray.map(classId => quizService.getQuizzes({ classId }))).then(results => results.flat()) : Promise.resolve([]),
          classIdsArray.length > 0 ? Promise.all(classIdsArray.map(classId => scheduleService.getSchedules({ classId }))).then(results => results.flat()) : Promise.resolve([]),
        ]);
        ...
        assignmentsData.forEach(assignment => {
          calendarEvents.push({
            ...
            color: '#ff3b30',
            route: ROUTES.TEACHER_ASSIGNMENTS,
          });
        });
        ...
        quizzesData.forEach(quiz => {
          calendarEvents.push({
            ...
            subject: subjectMap[quiz.subjectId],
            description: quiz.description,
            color: '#007aff',
            route: ROUTES.TEACHER_QUIZZES,
          });
        });
        ...
        schedulesData.forEach((schedule: any) => {
          ...
          calendarEvents.push({
            ...
            date: scheduleDate,
            subject: subjectName,
            description: `${schedule.startTime} - ${schedule.endTime} • ${className}`,
            color: '#5856d6',
  ...
  filteredEvents.forEach((event) => {
    ...
    const start = new Date(Math.max(eventDate.getTime(), new Date(year, month, 1).getTime()));
    const end = new Date(Math.min(eventEndDate.getTime(), new Date(year, month + 1, 0).getTime()));
